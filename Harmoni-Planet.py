...
def get_files(path):
    ...
    umd2_files  = np.array([i for i in image_files if "udm2" in os.path.basename(i)])
    ...
    image_files_sorted, umd2_files_sorted, metadata_files_sorted, name_id = [], [], [], []
    for i in metadata_files:
        ...
        corresponding_umd_file = np.where(np.char.find(umd2_files, cur_imgid) != -1)[0]
        if(len(corresponding_image_file)>0) and (len(corresponding_umd_file)>0):
            image_files_sorted.append(image_files[corresponding_image_file])
            umd2_files_sorted.append(umd2_files[corresponding_umd_file])
            metadata_files_sorted.append(i)
            name_id.append(cur_imgid)
    image_files_sorted, umd2_files_sorted, metadata_files_sorted = np.array(image_files_sorted).flatten(), np.array(umd2_files_sorted).flatten(), np.array(metadata_files_sorted).flatten()
    ...
    return image_files_sorted, umd2_files_sorted, footprint_names, footprint_objects
...
def initialize_graph(node_names, edge_names, edge_objects, image_files, udm2_files):
    ...
    for i,obj in enumerate(edge_objects):
        ...
        #Collect the invariant piTruexels from the intersection area.
        image_path_node1 = image_files[np.where(footprint_names == edge_names[i][0])][0]
        umd2_path_node1 = udm2_files[np.where(footprint_names == edge_names[i][0])][0]
        image_path_node2 = image_files[np.where(footprint_names == edge_names[i][1])][0]
        umd2_path_node2 = udm2_files[np.where(footprint_names == edge_names[i][1])][0]
        invariant_pixels_node1, invariant_pixels_node2 = get_invariant_pixels(sample_plots, image_path_node1, umd2_path_node1, image_path_node2, umd2_path_node2)

        #If no (or very few) valid invariant pixels within this region.
        if(len(invariant_pixels_node1) == 0) or (len(invariant_pixels_node1[0]) < 10):
            print("")
            continue
...
def get_invariant_pixels(sample_plots, image_path_node1, umd2_path_node1, image_path_node2, umd2_path_node2):
    #Link to images
    image_ds_node1 = gdal.Open(image_path_node1)
    udm_ds_node1 = gdal.Open(umd2_path_node1)
    ...
    udm_ds_node2 = gdal.Open(umd2_path_node2)
    ...
    for sp in sample_plots:
        ...
        if(width<=0 or height<=0):
            continue #invalid read-in range
        #Import the udm data
...
def export_harmonized_image(harmimg_folder, imagefile_folder, node_names, clusters, Harm_k, Harm_b, harm_list = -1):
    ...
    image_files, umd2_files, footprint_names, footprint_objects = get_files(imagefile_folder)
    main_cluster_id, main_cluster_node_count, other_clusters_node_count, isolated_node_count = cluster_statistic(clusters)
    for i in range(len(image_files)):
        ...
        umd2_file = umd2_files[i]
        img_ds = gdal.Open(img_file)
        umd2_ds = gdal.Open(umd2_file)
        ...
        for x in range(0, Cur_Columns, Block_size): 
            ...
            for y in range(0, Cur_Rows, Block_size): 
                ...
                umd2_data = umd2_ds.GetRasterBand(1).ReadAsArray(x, y, cols, rows)
                if(bandcount == 4):
                    #Four-band image
                    for c in channels:
                        if c == "Blue":
                        ...
                        image_data[:,:,c_id][np.where(umd2_data == 0)] = 0
...
imagefile_folder = r"E:\Planet_Data\Beijing\Test_Data"
...
nodefile_path = r"E:\Planet_Data\Beijing\Test_Graph\node_opt.csv"        # Path to the node data file
edgefile_path = r"E:\Planet_Data\Beijing\Test_Graph\edge_opt.csv"        # Path to the edge data file
invarpx_path = r"E:\Planet_Data\Beijing\Test_Graph\invar_opt.csv"        # Path to the invariant pixel data file
# Specify the selected channels to be harmonized.
channels = ["Coastal Blue", "Blue", "Green I", "Green II", "Yellow", "Red", "Red-Edge", "NIR"] #Eight bands, for SuperDove
#channels = ["Blue", "Green", "Red", "NIR"] #Four bands, for Dove Classic and Dove R
...
optfile_folder = r"E:\Planet_Data\Beijing\Test_Graph"
# Specify the folder to store the masked harmonized images.
harm_folder = r"E:\Planet_Data\Beijing\Test_corrected"
...
image_files, udm2_files, footprint_names, footprint_objects = get_files(imagefile_folder)
...
edge_names, edge_attributes, edge_invar_pxs, node_attributes = initialize_graph(node_names, edge_names, edge_objects, image_files, udm2_files)
