    ...
    for sp in sample_plots:
        ...
        if(width > 0 and height > 0):
            ...
            if len(np.where((udm_data_node1 == 1)&(udm_data_node2 == 1))[0])/(height*width) >= clear_threshold:
                ...
                #Mask out invalid pixels (where udm_data neq 1, or image_data filled with 1)
                image_data_node1_masked = np.copy(image_data_node1)[:,np.where((udm_data_node1 == 1) & (udm_data_node2 == 1) & (np.any(image_data_node1 == 1, axis = 0) == False) & (np.any(image_data_node2 == 1, axis = 0) == False))[0]]
                image_data_node2_masked = np.copy(image_data_node2)[:,np.where((udm_data_node1 == 1) & (udm_data_node2 == 1) & (np.any(image_data_node1 == 1, axis = 0) == False) & (np.any(image_data_node2 == 1, axis = 0) == False))[0]]
...
imagefile_folder = r"D:\Planet_Harmonization\Figure_code\Opensource_2025Jul\Test_Data_Beijing"
...
nodefile_path = r"E:\Planet_Data\Beijing\Test_Harmonization\node_test.csv"        # Path to the node data file
edgefile_path = r"E:\Planet_Data\Beijing\Test_Harmonization\edge_test.csv"        # Path to the edge data file
invarpx_path = r"E:\Planet_Data\Beijing\Test_Harmonization\invar_test.csv"        # Path to the invariant pixel data file
...
optfile_folder = r"E:\Planet_Data\Beijing\Test_Harmonization"
# Specify the folder to store the masked harmonized images.
harm_folder = r"E:\Planet_Data\Beijing\Test_Harmonization\Test_Corrected"
